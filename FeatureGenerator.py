class FeatureGenerator:
    ...
    def generate_feature(filename, all_paths):
        with open(filename, "w") as file:
            for index, path in enumerate(all_paths):
                FeatureGenerator._generate_test_case(file, path, index)

    @staticmethod
    def _generate_test_case(file, path, index):
        file.write(f"Scenario: scenario {index}\n")
        for index, node in enumerate(path):
            FeatureGenerator._generate_prefix(index, file)
            FeatureGenerator._generate_step(node, file)
        file.write("\n")
    ...
    @staticmethod
    def _generate_prefix(index, file):
        if index == 0:
            file.write("Given ")
        if (index + 1) % 2 == 0:
            file.write("When ")
